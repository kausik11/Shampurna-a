    ...
    >
      <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center xl:gap-12">
        <div className="reveal space-y-7 sm:space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {aboutHighlights.map((item) => (
              <GlassPanel key={item.title} className="reveal p-4 sm:p-5">
                <p className="text-[10px] uppercase tracking-[0.14em] text-[var(--color-gold)] min-[380px]:text-[11px] sm:text-xs sm:tracking-[0.24em]">
                  {item.label}
                </p>
          ...
          <blockquote className="reveal border-l border-[rgba(252,223,92,0.35)] pl-4 font-display text-xl leading-relaxed text-[var(--color-heading)] sm:pl-5 sm:text-2xl lg:text-3xl">
